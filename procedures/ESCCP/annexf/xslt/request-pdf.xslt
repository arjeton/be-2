<xsl:stylesheet
    ...
    <xsl:template name="html">
        <html>
            <head>
                ...
                <title>
                    <xsl:value-of select="ext:translate('spc.request.pdf.main.title', $pdfLanguage, $configVersion)" />
                </title>
            </head>
            <body>
    ...
    <xsl:template match="map">
        ...
        <div id="footer-left">
          <p>
            <xsl:value-of select="ext:translate('spc.request.pdf.footer', $pdfLanguage, $configVersion)" />
          </p>
        </div>
        <div id="footer-right">
        ...
        <main>
            ...
            <xsl:call-template name="nonEmptyFieldWithLabel">
                <xsl:with-param name="label" select="ext:translate('spc.additional.notes.title', $pdfLanguage, $configVersion)"/>
                <xsl:with-param name="fieldText" select="map[@key='additionalNotes']/string[@key='additionalNotes']"/>
            </xsl:call-template>
    ...
    <xsl:template name="adminTable">
        ...
        <xsl:call-template name="checkBoxWithLabel">
            <xsl:with-param name="label" select="ext:translate('spc.request.pdf.grant.patent', $pdfLanguage, $configVersion)" />
    ...
    <xsl:template name="basicFilingInfo">
            ...
            <tr>
                ...
                <td>
                    <xsl:call-template name="fieldWithLabel">
                        <xsl:with-param name="label" select="ext:translate('basic-filing.user-reference', $pdfLanguage, $configVersion)"/>
        ...
        <table class="content">
            <tr>
             <td class="referenceCell"/>
              <td>
               <label>
                    <xsl:value-of select="ext:translate('spc.request.pdf.parties.request.type', $pdfLanguage, $configVersion)" />
                </label>
              </td>
            </tr>
            <tr>
                <td class="referenceCell"/>
                <td>
                    <xsl:if test="map[@key='basicFilingInfo']/string[@key='requestType'] = ('PLANT_PROTECTION_PRODUCT') " >
                        <xsl:value-of select="ext:translate('spc.request.pdf.plant.protection.product', $pdfLanguage, $configVersion)" />
                    </xsl:if>
                    <xsl:if test="map[@key='basicFilingInfo']/string[@key='requestType'] = ('MEDICINAL_PRODUCT') " >
                        <xsl:value-of select="ext:translate('spc.request.pdf.medicinal.product', $pdfLanguage, $configVersion)" />
                    </xsl:if>
                    <xsl:if test="map[@key='basicFilingInfo']/string[@key='requestType'] = ('REQUEST_EXTENSION_PEDIATRIC_USE') " >
                        <xsl:value-of select="ext:translate('spc.request.pdf.request.extention.pediatric.use', $pdfLanguage, $configVersion)" />
                    </xsl:if>
                </td>
            </tr>
        </table>

        <div class="sectionDivider-small"/>
            ...
            <tr>
                ...
                <td>
                    <xsl:call-template name="nonSelectedCheckBoxWithLabel">
                        <xsl:with-param name="checkboxValue" select="map[@key='basicFilingInfo']/boolean[@key='pediatricExtension'] = true()" />
                        <xsl:with-param name="label" select="ext:translate('spc.request.pdf.request.extention.pediatric.use', $pdfLanguage, $configVersion)" />
                    </xsl:call-template>
                </td>
            </tr>
        </table>
        ...
        <table class="content">
            <tr>
             <td class="referenceCell"/>
              <td>
               <label>
                    <xsl:value-of select="ext:translate('spc.request.pdf.parties.patent.type', $pdfLanguage, $configVersion)" />
                </label>
              </td>
            </tr>
            <tr>
                <td class="referenceCell"/>
                <td>
                    <xsl:if test="map[@key='basicFilingInfo']/string[@key='patentType'] = ('PATENT_TYPE_NATIONAL') " >
                        <xsl:value-of select="ext:translate('spc.request.pdf.parties.patent.type.national', $pdfLanguage, $configVersion)" />
                    </xsl:if>
                    <xsl:if test="map[@key='basicFilingInfo']/string[@key='patentType'] = ('PATENT_TYPE_EUROPEAN') " >
                        <xsl:value-of select="ext:translate('spc.request.pdf.parties.patent.type.european', $pdfLanguage, $configVersion)" />
                    </xsl:if>
                </td>
            </tr>
        </table>
        ...
        <table class="content">
            <tr>
                ...
                <td>
                    <xsl:call-template name="fieldWithLabel">
                        <xsl:with-param name="label" select="ext:translate('request.pdf.application.number', $pdfLanguage, $configVersion)"/>
                        <xsl:with-param name="fieldText" select="map[@key='basicFilingInfo']/map[@key='applicationNumberContainer']/string[@key='applicationNumber']"/>
                    </xsl:call-template>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <xsl:call-template name="fieldWithLabel">
                        <xsl:with-param name="label" select="ext:translate('validation.property.publication-number', $pdfLanguage, $configVersion)"/>
                        <xsl:with-param name="fieldText" select="map[@key='basicFilingInfo']/map[@key='applicationNumberContainer']/string[@key='publicationNumber']"/>
                    </xsl:call-template>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <xsl:call-template name="fieldWithLabel">
                        <xsl:with-param name="label" select="ext:translate('basic-filing.filing-date', $pdfLanguage, $configVersion)"/>
                        <xsl:with-param name="fieldText" select="map[@key='basicFilingInfo']/string[@key='filingDate']"/>
                    </xsl:call-template>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <xsl:call-template name="fieldWithLabel">
                        <xsl:with-param name="label" select="ext:translate('basic-filing.grant-date', $pdfLanguage, $configVersion)"/>
                        <xsl:with-param name="fieldText" select="map[@key='basicFilingInfo']/string[@key='grantDate']"/>
                    </xsl:call-template>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td>
                    <xsl:call-template name="fieldWithLabel">
                        <xsl:with-param name="label" select="ext:translate('basic-filing.product-name', $pdfLanguage, $configVersion)"/>
                        <xsl:with-param name="fieldText" select="map[@key='basicFilingInfo']/string[@key='productName']"/>
                    </xsl:call-template>
                </td>
            </tr>
        </table>
            ...
            <table class="content" style="page-break-inside: avoid;">
                <tr>
                    ...
                    <td>
                        ...
                        <xsl:if test="not(map[@key='parties']/array[@key='applicants'] != '' or map[@key='parties']/array[@key='representatives'] != '' or map[@key='parties']/array[@key='communicationAddress'] = '')" >
        ...
        <div>
            <table class="content" style="page-break-inside: avoid;">
                <tr>
                    ...
                    <td>
                        <h1>
                            <xsl:value-of select="ext:translate('spc.request.pdf.declarations', $pdfLanguage, $configVersion)" />
                        </h1>
                        <h4>
                            <xsl:value-of select="ext:translate('spc.request.pdf.declarations.subtitle', $pdfLanguage, $configVersion)" />
                        </h4>
                        <xsl:if test="not(map[@key='declaration']/array[@key='marketingDeclarations'] != '')" >
                            <xsl:value-of select="ext:translate('request.pdf.data.not.entered', $pdfLanguage, $configVersion)"/>
                        </xsl:if> 
                        <xsl:if test="map[@key='declaration']/array[@key='marketingDeclarations'] != ''" >
                            <div class="subSectionDivider"/>
                            <xsl:call-template name="marketingDeclarations">
                                <xsl:with-param name="marketingDeclarations" select="map[@key='declaration']/array[@key='marketingDeclarations']"/>
                            </xsl:call-template>
                        </xsl:if>
                    </td>
                </tr>
    ...
    <xsl:template name="marketingDeclaration">
        <xsl:param name="marketingDeclaration" />
            <table class="content">
                    <tr>
                        <td class="referenceCell">
                            <div  class="reference">
                        ...
                        <td>
                            <label>
                                <xsl:value-of select="ext:translate('declaration.marketing.authority', $pdfLanguage, $configVersion)" />
                            </label>
                        </td>
                    </tr>
                    <tr>
                        <td class="referenceCell"/>
                        <td>
                            <xsl:if test="$marketingDeclaration/string[@key='authority'] = ('AUTHORITY_EUROPEAN') " >
                                <xsl:value-of select="ext:translate('PATENT_TYPE_EUROPEAN', $pdfLanguage, $configVersion)" />
                            </xsl:if>
                            <xsl:if test="$marketingDeclaration/string[@key='authority'] = ('AUTHORITY_NATIONAL') " >
                                <xsl:value-of select="ext:translate('AUTHORITY_NATIONAL', $pdfLanguage, $configVersion)" />
                            </xsl:if>
                        </td>
                    </tr>
                    <div class="sectionDivider-small"/>
                    <tr>
                        ...
                        <td>
                            <xsl:call-template name="fieldWithLabel">
                                <xsl:with-param name="label" select="ext:translate('validation.property.authorisation-number', $pdfLanguage, $configVersion)"/>
                                <xsl:with-param name="fieldText" select="$marketingDeclaration/string[@key='authorizationNumber']"/>
                            </xsl:call-template>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <xsl:call-template name="fieldWithLabel">
                                <xsl:with-param name="label" select="ext:translate('validation.property.authorisation-date', $pdfLanguage, $configVersion)"/>
                                <xsl:with-param name="fieldText" select="$marketingDeclaration/string[@key='authorizationDate']"/>
                            </xsl:call-template>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <xsl:call-template name="fieldWithLabel">
                                <xsl:with-param name="label" select="ext:translate('declaration.marketing.publication-date', $pdfLanguage, $configVersion)"/>
                                <xsl:with-param name="fieldText" select="$marketingDeclaration/string[@key='publicationDate']"/>
                            </xsl:call-template>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <xsl:call-template name="fieldWithLabel">
                                <xsl:with-param name="label" select="ext:translate('declaration.marketing.product-name', $pdfLanguage, $configVersion)"/>
                                <xsl:with-param name="fieldText" select="$marketingDeclaration/string[@key='productName']"/>
                            </xsl:call-template>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <label>
                                <xsl:value-of select="ext:translate('spc.request.pdf.declarations.markrting.authorisation', $pdfLanguage, $configVersion)" />
                            </label>
                        </td>
                    </tr>
                    <tr>
                        <td class="referenceCell"/>
                        <td>
                            <xsl:if test="$marketingDeclaration/string[@key='firstMarketingAuthorisation'] = ('FIRST_MARKETING_AUTHORISATION_YES') " >
                                <xsl:value-of select="ext:translate('yes', $pdfLanguage, $configVersion)" />
                            </xsl:if>
                            <xsl:if test="$marketingDeclaration/string[@key='firstMarketingAuthorisation'] = ('FIRST_MARKETING_AUTHORISATION_NO') " >
                                <xsl:value-of select="ext:translate('no', $pdfLanguage, $configVersion)" />
                            </xsl:if>
                        </td>
                    </tr>
                    <div class="sectionDivider-small"/>
                    <xsl:if test="$marketingDeclaration/string[@key='firstMarketingAuthorisation'] != 'FIRST_MARKETING_AUTHORISATION_YES'" >
                        <tr>
                            ...
                            <td>
                                <xsl:call-template name="fieldWithLabel">
                                    <xsl:with-param name="label" select="ext:translate('request.pdf.country', $pdfLanguage, $configVersion)"/>
                                    <xsl:with-param name="fieldText" select="ext:translate_enum($marketingDeclaration/map[@key='firstMarketingAuthorisationContainer']/string[@key='country'], 'procedures-common-form-geo.jsonschema-definitions-country', $pdfLanguage, $configVersion, $procedure)"/>
                                </xsl:call-template>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <xsl:call-template name="fieldWithLabel">
                                    <xsl:with-param name="label" select="ext:translate('validation.property.authorisation-number', $pdfLanguage, $configVersion)"/>
                                    <xsl:with-param name="fieldText" select="$marketingDeclaration/map[@key='firstMarketingAuthorisationContainer']/string[@key='authorizationNumber']"/>
                                </xsl:call-template>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <xsl:call-template name="fieldWithLabel">
                                    <xsl:with-param name="label" select="ext:translate('validation.property.authorisation-date', $pdfLanguage, $configVersion)"/>
                                    <xsl:with-param name="fieldText" select="$marketingDeclaration/map[@key='firstMarketingAuthorisationContainer']/string[@key='authorizationDate']"/>
                                </xsl:call-template>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <xsl:call-template name="fieldWithLabel">
                                    <xsl:with-param name="label" select="ext:translate('eclaration.marketing.publication-date', $pdfLanguage, $configVersion)"/>
                                    <xsl:with-param name="fieldText" select="$marketingDeclaration/map[@key='firstMarketingAuthorisationContainer']/string[@key='notificationDate']"/>
                                </xsl:call-template>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <xsl:call-template name="fieldWithLabel">
                                    <xsl:with-param name="label" select="ext:translate('declaration.marketing.product-name', $pdfLanguage, $configVersion)"/>
                                    <xsl:with-param name="fieldText" select="$marketingDeclaration/map[@key='firstMarketingAuthorisationContainer']/string[@key='productName']"/>
                                </xsl:call-template>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <xsl:call-template name="fieldWithLabel">
                                    <xsl:with-param name="label" select="ext:translate('validation.property.legal-bases', $pdfLanguage, $configVersion)"/>
                                    <xsl:with-param name="fieldText" select="ext:translate_enum($marketingDeclaration/map[@key='firstMarketingAuthorisationContainer']/string[@key='legalBases'], 'procedures-common-form-geo.jsonschema-definitions-country', $pdfLanguage, $configVersion, $procedure)"/>
                                </xsl:call-template>
                            </td>
                        </tr>
                    </xsl:if>
    ...
    <xsl:template name="marketingDeclarations">
        ...
        <div class="container">
            <xsl:for-each select="$marketingDeclarations/map" >
                <table class="content">
                    <tr>
                        <td>
                            <xsl:call-template name="marketingDeclaration">
                                <xsl:with-param name="marketingDeclaration" select="current()"/>
                            </xsl:call-template>
                        </td>
                    </tr>
                </table>
    ...
    <xsl:template name="attachments">
        ...
        <div style="page-break-inside: avoid;">
            <!--Title and subtitle for SPC Attachments -->
            <table class="content">
                <tr>
                    <td class="referenceCell">
                    </td>
                    <td>
                        <h1>
                            <xsl:value-of select="ext:translate('attachments.title', $pdfLanguage, $configVersion)"/>
                        </h1>
                    </td>
                </tr>
            </table>
            ...
            <div class="attachmentsTable">
                <table>
                    <thead>
                        <tr>
                            <th>
                                <xsl:value-of select="ext:translate('spc.request.pdf.attachments.type', $pdfLanguage, $configVersion)" />
                            </th>
                            <th>
                                <xsl:value-of select="ext:translate('request.pdf.country', $pdfLanguage, $configVersion)" />
                            </th>
                            <th>
                                <xsl:value-of select="ext:translate('request.pdf.original.file.name', $pdfLanguage, $configVersion)" />
                            </th>
                            <th>
                    ...
                    <tbody>
                        <xsl:if test="not($attachments/map[string[@key='attachmentSubType'] = 'SPC'])">
                            <tr>
                        ...
                        <xsl:for-each select="$attachments/map[string[@key='attachmentSubType'] = 'SPC']">
                                <tr>
                                    <td>
                                        <xsl:value-of select="ext:translate(concat('attachments.type.', current()/string[@key='attachmentType']), $pdfLanguage, $configVersion)" />
                                    </td>
                                    <td>
                                        <xsl:value-of select="current()/string[@key='country']" />
                                    </td>
                                    <td>
    ...
    <xsl:template name="applicant">
        ...
        <div class="container">
            ...
            <xsl:call-template name="nonEmptyFieldWithLabel">
                <xsl:with-param name="label" select="ext:translate('spc.parties.communicationAddress.company', $pdfLanguage, $configVersion)"/>
                <xsl:with-param name="fieldText" select="$applicant/map[@key='company']/string[@key='company']"/>
            </xsl:call-template>

            <xsl:call-template name="nonEmptyFieldWithLabel">
                <xsl:with-param name="label" select="ext:translate('parties.department', $pdfLanguage, $configVersion)"/>
                <xsl:with-param name="fieldText" select="$applicant/map[@key='company']/string[@key='department']"/>
            </xsl:call-template>

            
            <xsl:call-template name="nonEmptyFieldWithLabel">
                <xsl:with-param name="label" select="ext:translate('parties.address', $pdfLanguage, $configVersion)"/>
                <xsl:with-param name="fieldText" select="$applicant/map[@key='contactDetails']/map[@key='residenceAddress']/string[@key='streetAddress']"/>
            </xsl:call-template>

            <xsl:call-template name="postalDetails">
                <xsl:with-param name="contactDetails" select="$applicant/map[@key='contactDetails']/map[@key='residenceAddress']"/>
            </xsl:call-template>

            <xsl:call-template name="nonEmptyFieldWithLabel">
                <xsl:with-param name="label" select="ext:translate('parties.city', $pdfLanguage, $configVersion)"/>
                <xsl:with-param name="fieldText" select="$applicant/map[@key='contactDetails']/map[@key='residenceAddress']/string[@key='city']"/>
            </xsl:call-template>

            <xsl:call-template name="nonEmptyFieldWithLabel">
                <xsl:with-param name="label" select="ext:translate('request.pdf.country', $pdfLanguage, $configVersion)"/>
                <xsl:with-param name="fieldText" select="ext:translate_enum($applicant/map[@key='contactDetails']/map[@key='residenceAddress']/string[@key='country'], 'procedures-common-form-geo.jsonschema-definitions-country', $pdfLanguage, $configVersion, $procedure)"/>
            </xsl:call-template>

            <xsl:call-template name="nonEmptyFieldWithLabel">
                <xsl:with-param name="label" select="ext:translate('request.pdf.region', $pdfLanguage, $configVersion)"/>
                <xsl:with-param name="fieldText" select="$applicant/map[@key='contactDetails']/map[@key='residenceAddress']/string[@key='region']"/>
            </xsl:call-template>
    ...
    <xsl:template name="companyDetails">
        ...
        <xsl:call-template name="fieldWithLabel">
            <xsl:with-param name="label" select="ext:translate('request.pdf.idNumber', $pdfLanguage, $configVersion)"/>
    ...
    <xsl:template name="companyDetailGeneral">
        <xsl:param name="company"/>
        
        <xsl:if test="$company">
            <xsl:if test="$company/string[@key='company']">
                <div>
                    <div class="input-item">
                        <xsl:call-template name="nonEmptyFieldWithLabel">
                            <xsl:with-param name="label" select="ext:translate('spc.parties.communicationAddress.company', $pdfLanguage, $configVersion)"/>
                            <xsl:with-param name="fieldText" select="$company/string[@key='company']"/>
                            <xsl:with-param name="customFieldClass" select="'input'" />
                        </xsl:call-template>
                    </div>
                    <div class="input-item">
                        <xsl:call-template name="nonEmptyFieldWithLabel">
                            <xsl:with-param name="label" select="ext:translate('request.pdf.general.authorisation', $pdfLanguage, $configVersion)"/>
                            <xsl:with-param name="fieldText" select="$company/string[@key='generalAuthorization']"/>
                            <xsl:with-param name="customFieldClass" select="'input'" />
                        </xsl:call-template>
                    </div>
                </div>
    ...
    <xsl:template name="phoneDetails">
        ...
        <xsl:if test="$contactDetails">
            <xsl:if test="$contactDetails/string[@key='telephone'] or $contactDetails/string[@key='otherTelephone']">
        ...
        <xsl:if test="$contactDetails">
            <xsl:if test="$contactDetails/string[@key='email']">
                    ...
                    <div class="input-item">
                        <xsl:call-template name="nonEmptyFieldWithLabel">
                            <xsl:with-param name="label" select="ext:translate('request.pdf.email', $pdfLanguage, $configVersion)"/>
                            <xsl:with-param name="fieldText" select="$contactDetails/string[@key='email']"/>
                            <xsl:with-param name="customFieldClass" select="'input'" />
                        </xsl:call-template>
                    </div>
    ...
    <xsl:template name="postalDetails">
        <xsl:param name="contactDetails"/>
        
        <xsl:if test="$contactDetails">
            <xsl:if test="$contactDetails/string[@key='postalCode']">
                <div class="input-flex">
                    <div class="input-item-close">
                        <xsl:call-template name="nonEmptyFieldWithLabel">
                            <xsl:with-param name="label" select="ext:translate('parties.postal-code', $pdfLanguage, $configVersion)"/>
                            <xsl:with-param name="fieldText" select="$contactDetails/string[@key='postalCode']"/>
                            <xsl:with-param name="customFieldClass" select="'input-medium'" />
                        </xsl:call-template>
                    </div>
                    <div class="input-item">
                        <xsl:call-template name="nonEmptyFieldWithLabel">
                            <xsl:with-param name="label" select="ext:translate('parties.PO-box', $pdfLanguage, $configVersion)"/>
                            <xsl:with-param name="fieldText" select="$contactDetails/string[@key='poBox']"/>
                            <xsl:with-param name="customFieldClass" select="'input-medium'" />
                        </xsl:call-template>
                    </div>
                </div>
