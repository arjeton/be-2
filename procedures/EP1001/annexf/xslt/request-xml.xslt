<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="parties">
        ...
        <parties>
            <xsl:call-template name="applicants">
                ...
                <xsl:with-param name="employees" select="$object/array[@key='employees']"/>
                <xsl:with-param name="config" select="$config"/>
            </xsl:call-template>
    ...
    <xsl:template name="applicants">
        ...
        <applicants>
            ...
            <xsl:for-each select="$employees/map">
                <xsl:call-template name="applicant">
                    <xsl:with-param name="object" select="current()"/>
                    <xsl:with-param name="appType" select="'employee'"/>
                    <xsl:with-param name="config" select="$config"/>
                </xsl:call-template>
            </xsl:for-each>
        </applicants>
    ...
    <xsl:template name="agent">
        ...
        <xsl:variable name="repType">
            <xsl:if test="$object/string[@key='role'] = 'LEGAL_PRACTITIONER'">
                <xsl:value-of select="'attorney'"/>
            </xsl:if>
            <xsl:if test="$object/string[@key='role'] = 'ASSOCIATION'">
                <xsl:value-of select="'common-representative'"/>
            </xsl:if>
            <xsl:if test="$object/string[@key='role'] = 'PROFESSIONAL_REPRESENTATIVE'">
        ...
        <check-list>
            ...
            <xsl:for-each select="$attachments/map">
                ...
                <xsl:if test="current()/string[@key='attachmentType'] = 'PROOF_OF_PAYMENT'">
                    <xsl:call-template name="checklistItemOnlyTotalPages">
                        <xsl:with-param name="checkListTagName" select="'cl-paymemt-document'"/>
                        <xsl:with-param name="attachment" select="current()"/>
                    </xsl:call-template>
                </xsl:if>
            </xsl:for-each>
        ...
        <addressbook>
            ...
            <xsl:if test="$object/map[@key='company']/string[@key='company'] != ''">
                <xsl:call-template name="businessInfo">
                    ...
                    <xsl:with-param name="department">
                        <xsl:value-of select="$object/map[@key='company']/string[@key='department']"/>
                    </xsl:with-param>
                </xsl:call-template>
            </xsl:if>
            ...
            <xsl:call-template name="addressDetail">
                ...
                <xsl:with-param name="state">
                    <xsl:value-of select="$object/map[@key='contactDetails']/map[@key='residenceAddress']/string[@key='department']"/>
                </xsl:with-param>
