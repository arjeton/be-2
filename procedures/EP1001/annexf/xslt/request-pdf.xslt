<xsl:stylesheet
                    ...
                    <td>
                        ...
                        <xsl:for-each select="$biologicalInfos/map">
                            ...
                            <xsl:call-template name="fieldWithLabel">
                                ...
                                <!-- this enum does not exist in EP1001 schema.jsonschema local or external definitions -->
                            ...
                            <xsl:call-template name="nonEmptyFieldWithLabel">
                                <xsl:with-param name="label" select="ext:translate('request.pdf.bioinfo.uid.reference', $pdfLanguage, $configVersion)"/>
                                <xsl:with-param name="fieldText" select="ext:translate_enum($bioMaterial/string[@key='userId'], 'country', $pdfLanguage, $configVersion, $procedure)"/>
                            </xsl:call-template>
                            <xsl:call-template name="fieldWithLabel">
                                <xsl:with-param name="label" select="ext:translate('request.pdf.bioinfo.accession', $pdfLanguage, $configVersion)"/>
                                <xsl:with-param name="fieldText" select="ext:translate_enum($bioMaterial/string[@key='accession'], 'country', $pdfLanguage, $configVersion, $procedure)"/>
                            </xsl:call-template>
                            <xsl:call-template name="nonEmptyFieldWithLabel">
                            ...
                            <xsl:call-template name="nonEmptyFieldWithLabel">
                                <xsl:with-param name="label" select="ext:translate('request.pdf.bioinfo.origin', $pdfLanguage, $configVersion)"/>
                                <xsl:with-param name="fieldText" select="ext:translate_enum($bioMaterial/string[@key='origin'], 'country', $pdfLanguage, $configVersion, $procedure)"/>
                            </xsl:call-template>
                            <xsl:call-template name="nonSelectedCheckBoxWithLabel">
        ...
        <xsl:for-each select="$employees/map" >
            <table style="page-break-inside: avoid;" class="content">
                <tr>
                    ...
                    <td>
                        <h3>
                            <xsl:value-of select="concat(ext:translate('request.pdf.employee', $pdfLanguage, $configVersion),' ', current()/number[@key='sequenceNumber'])"/>
                        </h3>
                    </td>
                </tr>
            </table>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <xsl:call-template name="nonEmptyFieldWithLabel">
                            ...
                            <xsl:with-param name="fieldText" select="current()/map[@key='personalDetails']/string[@key='generalAuthorization']"/>
                        </xsl:call-template>
                    </td>
                </tr>
            </table>
        ...
        <xsl:for-each select="$representatives/map" >
            ...
            <table class="content">
                <tr>
                    <td class="referenceCell"/>
                    <td>
                        <xsl:if test="current()/string[@key='role'] = ('PROFESSIONAL_REPRESENTATIVE','LEGAL_PRACTITIONER')">
                            <xsl:call-template name="personalDetails">
                                <xsl:with-param name="role" select="current()/string[@key='role']"/>
                                <xsl:with-param name="personalDetails" select="current()/map[@key='personalDetails']"/>
                            </xsl:call-template>
                            ...
                            <xsl:call-template name="companyDetails">
                                <xsl:with-param name="role" select="current()/string[@key='role']"/>
                                <xsl:with-param name="companyDetails" select="current()/map[@key='company']"/>
                            </xsl:call-template>
                        </xsl:if>
            ...
            <table class="content">
                <tr>
                    ...
                    <td>
                        ...
                        <xsl:call-template name="nonSelectedCheckBoxWithLabel">
                            <xsl:with-param name="checkboxValue" select="current()/boolean[@key='isDeceased']" />
                            <xsl:with-param name="label" select="ext:translate('request.pdf.inventor.deceased', $pdfLanguage, $configVersion)" />
                        </xsl:call-template>
                        ...
                        <xsl:call-template name="fieldWithLabel">
                            <xsl:with-param name="label" select="ext:translate('request.pdf.declaration.rights', $pdfLanguage, $configVersion)"/>
                            <xsl:with-param name="fieldText" select="ext:translate_enum(current()/string[@key='declarationOfRights'], 'country', $pdfLanguage, $configVersion, $procedure)"/>
                        </xsl:call-template>

                        <xsl:call-template name="nonEmptyFieldWithLabel">
                            <xsl:with-param name="label" select="ext:translate('request.pdf.date.of.entry', $pdfLanguage, $configVersion)"/>
                            <xsl:with-param name="fieldText" select="current()/string[@key='dateOfEntry']"/>
                        </xsl:call-template>
                    </td>
                </tr>
            </table>
    ...
    <xsl:template name="priorities">
        ...
        <table style="page-break-inside: avoid;">
            <thead>
                <tr>
                    ...
                    <th>
                        <xsl:value-of select="ext:translate('request.pdf.das.code', $pdfLanguage, $configVersion)"/>
                    </th>
                    <th>
                        <xsl:value-of select="ext:translate('request.pdf.type.of.office', $pdfLanguage, $configVersion)"/>
                    </th>
                    <th>
                        <xsl:value-of select="ext:translate('declaration.priority.regional-office', $pdfLanguage, $configVersion)"/>
                    </th>
                    <th>
            ...
            <tbody>
                <xsl:for-each select="$priorities/map">
                    <tr>
                        ...
                        <td>
                            <xsl:value-of select="current()/map[@key='dasCode']/string[@key='dasCodeValue']"/>
                        </td>
                        <td>
                            <xsl:value-of select="ext:translate_enum(current()/string[@key='officeType'], 'country', $pdfLanguage, $configVersion, $procedure)" />
                        </td>
                        <td>
                            <!-- this enum does not exist in EP1001 schema.jsonschema local or external definitions -->
                            <xsl:value-of select="ext:translate_enum(current()/string[@key='office'], 'priorityOffice', $pdfLanguage, $configVersion, $procedure)" />
                        </td>
                        <td>
                            <xsl:value-of select="ext:translate_enum(current()/string[@key='applicationKind'], 'country', $pdfLanguage, $configVersion, $procedure)" />
                        </td>
                        <td>
        ...
        <xsl:if test="$priorities/map/boolean['reestablishment'] = true()">
            <table class="content" style="page-break-inside: avoid;">
                <tr>
                    <td class="referenceCell"/>
                    <td>
    ...
    <xsl:template name="parties">
        ...
        <xsl:if test="map[@key='parties']/array[@key='employees'] != ''" >
            ...
            <xsl:call-template name="employees">
                <xsl:with-param name="employees" select="map[@key='parties']/array[@key='employees']"/>
            </xsl:call-template>
        </xsl:if>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <xsl:call-template name="fieldWithLabel">
                            ...
                            <xsl:with-param name="fieldText" select="ext:translate_enum($fees/string[@key='methodOfPayment'], 'country', $pdfLanguage, $configVersion, $procedure)" />
                        </xsl:call-template>
                    ...
                    <tbody>
                        ...
                        <xsl:for-each select="$attachments/map">
                            <xsl:if test="current()/string[@key='attachmentType'] = ('SPECIFIC', 'GENERAL', 'ELECTRONIC_PRIORITY', 'TRANSLATIONS_OF_TECHNICAL_DOCUMENTS', 'TRANSLATIONS_OF_PRIORITY_DOCUMENTS', 'INVENTOR_WAIVER',  'OTHER')">
                                <tr>
                                    <td>
                                        <xsl:value-of select="ext:translate_enum(current()/string[@key='attachmentType'], 'country', $pdfLanguage, $configVersion, $procedure)" />
                                    </td>
                                    <td>
                                        <xsl:value-of select="current()/string[@key='originalFileName']" />
                                    </td>
    ...
    <xsl:template name="applicant">
        ...
        <div class="container">
            ...
            <xsl:if test="$applicant/string[@key='role'] = ('APPLICANT_NATURAL_PERSON')">
                ...
                <xsl:call-template name="companyDetails">
                    <xsl:with-param name="role" select="$applicant/string[@key='role']"/>
                    <xsl:with-param name="companyDetails" select="$applicant/map[@key='company']"/>
                </xsl:call-template>
    ...
    <xsl:template name="personalDetails">
        ...
        <xsl:call-template name="title">
            <xsl:with-param name="label" select="ext:translate('request.pdf.title', $pdfLanguage, $configVersion)"/>
            <xsl:with-param name="fieldText" select="$personalDetails/string[@key='title']"/>
        </xsl:call-template>
        ...
        <xsl:call-template name="nonEmptyFieldWithLabel">
            ...
            <xsl:with-param name="fieldText" select="ext:translate_enum($personalDetails/string[@key='nationality'], 'country', $pdfLanguage, $configVersion, $procedure)"/>
        </xsl:call-template>
        
        <xsl:if test="$role = 'AGENT_NATURAL_PERSON'">
            <xsl:call-template name="fieldWithLabel">
                <xsl:with-param name="label" select="ext:translate('request.pdf.agent.code', $pdfLanguage, $configVersion)"/>
                <xsl:with-param name="fieldText" select="$personalDetails/string[@key='agentCode']"/>
            </xsl:call-template>
        </xsl:if>
    ...
    <xsl:template name="companyDetails">
        ...
        <xsl:if test="$role = ('APPLICANT_LEGAL_ENTITY', 'ASSOCIATION')">
            <xsl:call-template name="nonEmptyFieldWithLabel">
                ...
                <xsl:with-param name="fieldText" select="ext:translate_enum($companyDetails/string[@key='placeOfBusiness'], 'country', $pdfLanguage, $configVersion, $procedure)"/>
            </xsl:call-template>
        </xsl:if>
        ...
        <xsl:call-template name="nonEmptyFieldWithLabel">
            ...
            <xsl:with-param name="fieldText" select="ext:translate_enum(
                    $addressFields/string[@key='region'],
                     concat('region', $addressFields/string[@key='country']), $pdfLanguage,
                    $configVersion,
                    $procedure)"/>
        </xsl:call-template>
    </xsl:template>
